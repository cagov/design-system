    ...
    components.forEach((component) => {
      ...
      (['src', 'dist']).forEach((prefix) => {
        if (ctx.path.startsWith(`/${component.route}/_${prefix}/`)) {
          filePath = ctx.path.replace(`/${component.route}/_${prefix}/`, `${component.dir}/${prefix}/`);
        }
      });
      // Redefine the file path for special template-related paths.
      if (ctx.path.startsWith(`/${component.route}/_templates/`)) {
        filePath = ctx.path.replace(`/${component.route}/_templates/`, `${dirs.command}/templates/`);
      }
      // Otherwise, point to the examples folder.
      if (!filePath) {
        filePath = `${component.dir}/examples${ctx.path.replace(`/${component.route}`, '')}`;
      }
    });
